...
存在启用富媒体输出的 metadata?
  ...
  ↓
ZIP 命令?
  ├─ 是 -> 强制原视频/完整字段 -> 等待翻译 -> 在线程中 build_zip_archive()
...
主解析/下载会话使用 `create_public_only_connector()`：直连目标的 URL、DNS 结果、连接前 socket 地址和实际 peer 均必须是公网地址，重定向逐跳复验，跨源时剥离 Authorization/Cookie/Proxy-Authorization。显式配置的代理地址作为受信连接端点放行；代理模式仍拒绝字面私网目标，但普通域名最终解析和连接发生在代理端，属于管理员显式选择的代理信任边界。直连环境若使用 Clash/TUN `198.18.0.0/15` fake-IP 会 fail-closed，应改为真实 DNS/redir-host 或显式代理。
...
- `main.py` 在至少一条 metadata 启用富媒体输出时并发处理各条 metadata；开场语由锁保护的一次性回调按首个可发送媒体触发。
