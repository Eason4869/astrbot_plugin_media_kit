...
[![Version](https://img.shields.io/badge/Version-v7.1.0-green.svg)](https://github.com/Eason4869/astrbot_plugin_media_kit)
[![GitHub](https://img.shields.io/badge/作者-Eason4869-blue)](https://github.com/Eason4869)
...
本仓库基于 [astrbot_plugin_media_parser](https://github.com/drdon1234/astrbot_plugin_media_parser) 增强，在保留全部上游能力的基础上，主要改动：

- 🎴 **卡片渲染**：移植 [astrbot_plugin_rika_share](https://github.com/iris1598/astrbot_plugin_rika_share) 的卡片渲染器，将标题、作者、封面、统计等信息渲染为图片卡片，支持 `标准` / `杂志` / `沉浸式` / `信息流` 四种布局与深/浅主题（在 `消息输出 → 附加内容：卡片渲染` 中配置）；B站卡片内置播放/点赞/投币/收藏/转发/评论/弹幕等统计药丸与简洁 BV 号展示，封面下载失败时自动使用内置兜底背景，修改视觉样式后会自动失效旧缓存并重渲染。**解析流程为先发送卡片，再聚合发送视频/图片**
- 🤖 **群聊贴表情仲裁**：参考 [astrbot_plugin_parser](https://github.com/Zhalslar/astrbot_plugin_parser)，多 Bot 同群时通过贴表情弱一致仲裁，保证一条链接只有一个 Bot 解析（与该插件的 Bot 互通仲裁）；同时在链接消息上贴表情反馈解析状态（占坑👀 / 成功✅ / 失败❌），仅在 aiocqhttp（OneBot v11，如 NapCat/Lagrange）群聊生效
- 🎮 **新增 Steam 游戏页解析**：`store.steampowered.com/app/...`，默认走 Steam 官方接口，也可在设置中切换为小黑盒完整游戏路径
- 🔧 **修复小黑盒 / X(Twitter) 解析**：小黑盒改用签名游戏详情接口并修复 DES key；修复 X 解析头像字段缺失导致的失败
...
<td>游戏详情链接（<code>www.xiaoheihe.cn/app/topic/game/...</code>、<code>api.xiaoheihe.cn/game/share_game_detail?...</code>）<br>BBS 分享链接（<code>www.xiaoheihe.cn/app/bbs/link/...</code>）<br>小程序卡片（<code>message.meta.news.jumpUrl</code>）</td>
<td class="center">视频 / 图片 / 文本</td>
</tr>
<tr>
<td class="center"><strong>Steam</strong></td>
...
2. **插件**：打开 AstrBot WebUI → 插件市场搜索 `astrbot_plugin_media_kit` 并安装，或从 [GitHub 仓库](https://github.com/Eason4869/astrbot_plugin_media_kit) 手动安装
...
- ✅ 媒体中转模式，跨服务器部署无需共享目录
- ✅ 群聊贴表情仲裁（aiocqhttp/OneBot v11）：多 Bot 同群自动竞争解析权，贴表情反馈解析状态
- ✅ 支持 Steam 商店游戏页解析，可切换小黑盒完整游戏路径
...
在 aiocqhttp（OneBot v11，如 NapCat、Lagrange 等）平台的**群聊**中，插件会在被解析的链接消息上贴 QQ 表情，实现两件事：

- **多 Bot 仲裁**：同一条链接若被群内多个支持该协议的 Bot 同时看到，Bot 之间通过固定表情（占坑表情 289、胜出确认表情 124）的点赞用户列表做弱一致仲裁，最终只有一个 Bot 会解析并发送结果，避免重复刷屏。仲裁顺序按消息时间片确定性轮转，所有 Bot 结论一致；本协议与 [astrbot_plugin_parser](https://github.com/Zhalslar/astrbot_plugin_parser) 兼容，不同插件的 Bot 同群也能互相仲裁。
- **解析状态反馈**：Bot 识别到链接即贴占坑表情（表示「已接手」），解析成功贴成功表情，解析失败贴失败表情，方便判断解析是否在运行。

该能力无需配置，仅在支持 `set_msg_emoji_like` / `fetch_emoji_like` action 的协议端群聊中自动生效；私聊、其他平台或协议端不支持时自动跳过，不影响解析。

## 🎮 Steam 游戏页解析

`Steam` 输出模式默认开启。`store.steampowered.com/app/<appid>/...` 链接默认调用 Steam 官方 `appdetails` 接口获取游戏信息、截图与预告视频。在插件配置 `Steam 设置` 中开启「使用小黑盒路径解析 Steam 游戏页」后，会改用小黑盒完整游戏详情路径，可获得小黑盒评分、在线人数等更多统计信息（该路径复用小黑盒解析器）。Steam 媒体走 Steam CDN，下载速度不佳时可在 `代理设置 → Steam 代理` 中为解析/图片/视频分别启用代理。
